...
The `ResponsibleAgent` is the only place an `ApprovedOrder` is constructed. It signs the canonical JSON of `(decision_id, trade, approved_at)` with a 32-byte secret using HMAC-SHA256:

```python
def canonical_payload(decision_id, trade, approved_at):
    payload = {
...
def sign_trade(secret, decision_id, trade, approved_at):
...
```
{"approved_at":"2025-01-01T00:00:00+00:00","decision_id":"11111111-1111-4111-8111-111111111111","trade":{"analyst_id":"...","confidence":0.8,"limit_price":null,"order_type":"market","rationale":"...","side":"buy","size":0.001,"symbol":"BTC-USD","venue":"paper"}}
```

The signature is `HMAC-SHA256(secret, that-byte-string).hex()`. If anything in the trade changes (size, side, symbol, venue), the recomputed signature differs and the Executor refuses the order.
...
4. **Never logged.** A `RedactingFilter` is attached to every logger by `logging_setup.configure_logging`; it masks `secret_key`, `private_key`, `api_secret`, `passphrase`, `signer_private_key`, and `password` (case-insensitive) in both keyword args and free-form messages. See [security.md](security.md).
...
| Attack                                                              | Defence                                                                                                                                                                       |
| ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Propose an oversized trade                                          | `ResponsibleAgent` calls `risk.check_order` **before** asking the LLM for go/no-go. `max_notional_usd`, `max_leverage`, `symbol_allowlist`, `daily_loss_cap_usd` are all enforced in pure Python. |
...
| Tamper with an in-flight `ApprovedOrder` (change size, side, venue) | The HMAC covers `decision_id`, the full `trade` payload, and `approved_at`. Any change invalidates the signature.                                                              |
| Replay an old `ApprovedOrder`                                       | Each `decision_id` is a fresh UUID; the executor (and audit log) record which `decision_id` was placed. (Replay protection is a per-deployment concern; the audit log is the system of record.) |
| Bypass the live-trading gate                                        | Live mode requires both `--live` on the CLI **and** `run.live: true` (with `run.dry_run: false` and `credentials.live: true`) in the config. The config validator rejects inconsistent settings; the CLI rejects mismatched flags. |
| Exfiltrate credentials through logs                                 | The `RedactingFilter` on every handler masks sensitive keys and inline `name=value` pairs before the message is emitted. Test coverage in `tests/test_logging.py` exercises both shapes. |
...
| Cause silent failures                                               | The orchestrator catches per-phase exceptions, audit-logs them as `cycle_error`, and continues. The Executor audit-logs `executor_unauthorized` whenever it refuses a signature, which is the right signal for an alert. |
